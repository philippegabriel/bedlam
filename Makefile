...
targets+=xyzRotated.txt xyzRotated.sort.txt xyzRotated.sort.uniq.txt 
targets+=xyzTranslated.txt  xyzTranslated.sort.txt xyzTranslated.sort.uniq.txt
targets+=xyzlist.txt xyzvector.smt2 
targets+=bedlam.smt2 
#targets+=xyzListEncoded.txt xyzvectors.smt2 grid.smt2  
testtargets= xyzlist.sort.uniq.txt
...
xyzlist.txt: xyzTranslated.sort.uniq.txt
	perl xyz2xyzList.pl < $< > $@
xyzvector.smt2: xyzlist.sort.uniq.txt
	perl xyzList2smt2.pl  < $< > $@
bedlam.smt2: xyzvector.smt2
	sed -e '/;#include autogenerated/r$<' bedlam.smt2.in > $@
